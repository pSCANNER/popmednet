...
namespace Lpp.Scanner.DataMart.Model.Processors.Analysis {
    ...
    public static class RScriptWriter {
        ...
        static readonly Regex _queryMatch = new Regex(@"Query=(?<query>[\p{L}\s\*\[\]\._\d]+)(?:;|$)", RegexOptions.Compiled);
        ...
        public static string GenerateRScript(AggregatorClientRequestParameter parameter) {
            ...
            var linkFunction = parameter.Pmml.GetLinkFunction(LocalConstants.LinkFunctionName);
            var family = parameter.Pmml.GetFamily(LocalConstants.FamilyName);
