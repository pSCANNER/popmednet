...
namespace Lpp.Dns.Portal.Areas.Controls.Controllers
{
    public class WFFileUploadController : Controller
    {
        ...
        [HttpPost]
        public async Task<ActionResult> UploadResponseDocuments(IEnumerable<HttpPostedFileBase> files, string comments, Guid requestID, string authToken, Guid responseID, string documentType)
        {
            ...
            using (var web = new System.Net.Http.HttpClient() { Timeout = TimeSpan.FromMinutes(30) })
            {
                ...
                foreach (var f in files)
                {
                    ...
                    using (MultipartFormDataContent container = new MultipartFormDataContent())
                    {
                        ...
                        container.Add(new StringContent(documentType), "documentKind");
                        if (!string.IsNullOrWhiteSpace(comments))
                        {
                            container.Add(new StringContent(comments), "comments");
                        }
