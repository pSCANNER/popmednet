...
namespace Lpp.Dns.DataMart.Model.QueryComposer.Tests {
    ...
    public class QueryCompositionTests {
        ...
        public void Age_between_10_and_36_no_stratification() {
            ...
            // var q2 = query.Select(p => new { PatientID = p.ID, Age = (System.Data.Entity.DbFunctions.DiffYears(p.BornOn.Value, DateTime.Now).Value -
            // (((p.BornOn.Value.Month > DateTime.Now.Month) || (p.BornOn.Value.Month == DateTime.Now.Month && p.BornOn.Value.Day > DateTime.Now.Day)) ? 1 : 0))
            // }) .GroupBy(p => new { Age = p.Age }) .Select(p => new { Age = p.Key.Age, Count = p.Count() });
        ...
        public void AgeRange_AgeAtLastEncounterSystemWide() {
            ...
            // var result = query.GroupBy(k => new { k.Sex, k.Age }).Select(k => new { k.Key.Sex, k.Key.Age, Count = k.Count() }); foreach (var r in result) {
...
*/
        ...
        public void PMNDEV5330() {
            ...
            // var q2 = query.Select(p => new { PatientID = p.ID, BornOn = p.BornOn, //Age = (System.Data.Entity.DbFunctions.DiffYears(p.BornOn.Value,
            // DateTime.Now).Value - (((p.BornOn.Value.Month > DateTime.Now.Month) || (p.BornOn.Value.Month == DateTime.Now.Month && p.BornOn.Value.Day >
            // DateTime.Now.Day)) ? 1 : 0)) Age = (int?)((p.BornOn.Value > encounterStart) ? (DbFunctions.DiffYears(p.BornOn.Value, encounterStart).Value +
            // ((p.BornOn.Value.Month < encounterStart.Month || (p.BornOn.Value.Month == encounterStart.Month && p.BornOn.Value.Day < encounterStart.Day)) ? 1 :
            // 0)) : (DbFunctions.DiffYears(p.BornOn.Value, encounterStart).Value - (((p.BornOn.Value.Month > encounterStart.Month) || (p.BornOn.Value.Month ==
            // encounterStart.Month && p.BornOn.Value.Day > encounterStart.Day)) ? 1 : 0)) ) }) .OrderByDescending(p => p.BornOn); //.GroupBy(p => new { Age =
            // p.Age }) //.Select(p => new { Age = p.Key.Age, Count = p.Count() });

            // foreach (var p in q2) { Logger.Debug(string.Format("{0:D}\t\t{1}", p.BornOn, p.Age)); }
        ...
        public void PMNMAINT_1138_HPHCI_long_running_Oracle() {
            ...
            // var g = query.GroupBy(k => new { Hispanic = k.Hispanic, Race = k.Race, AdmittedOnYear = k.AdmittedOnYear }) .Select(k => new { Hispanic =
            // k.Key.Hispanic, Race = k.Key.Race, AdmittedOnYear = k.Key.AdmittedOnYear, Patients = k.Count() });

            // Logger.Debug(g.Expression.ToString()); Logger.Debug(g.ToString());
...
at OracleInternal.ServiceObjects.OracleCommandImpl.VerifyExecution(OracleConnectionImpl connectionImpl, Int32& cursorId, Boolean bThrowArrayBindRelatedErrors, OracleException& exceptionForArrayBindDML, Boolean& hasMoreRowsInDB, Boolean bFirstIterationDone)
 ...
 --- End of inner exception stack trace ---
   ...
   at Lpp.Dns.DataMart.Model.QueryComposer.Tests.QueryCompositionTests.WeightBetween_10_and_150_lb() in c:\LPP\PMN\Lpp.Adapters\Lpp.Dns.DataMart.Model.QueryComposer.Tests\QueryCompositionTests.cs:line 399
        ...
        private static string SerializeJsonToString(Lpp.Dns.DTO.QueryComposer.QueryComposerResponseDTO response) {
        ...
        private Lpp.Dns.DTO.QueryComposer.QueryComposerRequestDTO LoadRequest(string filename, string folder = ResourceFolder) {
        ...
        private Lpp.Dns.DTO.QueryComposer.QueryComposerResponseDTO RunRequest(string requestJsonFilepath) {
            return RunRequest(requestJsonFilepath, MSSqlConnectionString, Settings.SQLProvider.SQLServer);
        }

        private Lpp.Dns.DTO.QueryComposer.QueryComposerResponseDTO RunRequest(string requestJsonFilepath, string connectionString, Settings.SQLProvider sqlProvider) {
        ...
        private const bool RunMySql = false;
        private const bool RunOracle = false;
        private const bool RunPostgreSQL = false;
        private static readonly log4net.ILog Logger;
        private static readonly string MSSqlConnectionString;
        private static readonly string MySQLConnectionString;
        private static readonly string OracleConnectionString;
