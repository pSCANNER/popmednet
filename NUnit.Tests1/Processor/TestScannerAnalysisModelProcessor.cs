...
namespace Test.Lpp.pScanner.DataMart.Model.Processors.Processor {
    ...
    internal class TestScannerAnalysisModelProcessor : TestScannerAnalysisModelProcessorBase {
        ...
        public void Request_Documents_Empty_Test() {
            Assert.That(() => _processor.Request(It.IsAny<String>(), It.IsAny<NetworkConnectionMetadata>(), It.IsAny<RequestMetadata>(), null, out IDictionary<string, string> requestProperties, out Document[] desiredDocuments), Throws.Exception.TypeOf<ArgumentNullException>().With.Property("ParamName").EqualTo("Document[]"));
            Assert.That(() => _processor.Request(It.IsAny<String>(), It.IsAny<NetworkConnectionMetadata>(), It.IsAny<RequestMetadata>(), new Document[] { }, out IDictionary<string, string> requestProperties, out Document[] desiredDocuments), Throws.Exception.TypeOf<ArgumentNullException>().With.Property("ParamName").EqualTo("Document[]"));
        }
        ...
        public void Request_NetworkConnectionMetadata_Empty_Test() {
            Assert.That(() => _processor.Request(It.IsAny<String>(), null, It.IsAny<RequestMetadata>(), It.IsAny<Document[]>(), out IDictionary<string, string> requestProperties, out Document[] desiredDocuments), Throws.Exception.TypeOf<ArgumentNullException>().With.Property("ParamName").EqualTo("NetworkConnectionMetadata"));
        }
